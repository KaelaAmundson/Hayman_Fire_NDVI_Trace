...
#### NDSI summary 
```{r data exploring-ndsi, echo = FALSE}
summary(ndsi)
```
#### Q Summary 
```{r data exploring-q, echo = FALSE}
summary(q_hayman)
```
...
#### Q water year
```{r adding water year, echo = FALSE}
...
#### NDSI water year
```{r adding water year for NDSI, echo = TRUE}
...
```{r snow water sum stats, echo = FALSE}
...
``` {r whole q water year sum, echo = FALSE}
...
print(q_means)
```
## Plots of Snow Covers vs Q

### Mean Snow vs Median Q
``` {r plotting mean snow vs median Q, echo = FALSE}
snow_Q <- inner_join(snow_means, q_means, by=c('wtr_yr'))
...
ggplot(snow_Q, aes(x=mean_ndsi, y=median_q, color=wtr_yr)) +
  ...
  ylab("Median Dischange (cfs)")
```

### Max Snow vs. Median Q
``` {r max snow vs median q, echo = FALSE}
